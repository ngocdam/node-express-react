...
AltCoin.prototype.createRawTransaction = function(txParams) {
    ...
    for (var i = 0; i < listUnspent.length; i++) {
        ...
        //	break;
        //}
        if (typeof txParams.fee != "undefined" || txParams.fee != null) {
            transaction_fee = AltCoin.convertToSatoshi(txParams.fee);
        } else {
 ...
 * Validate address
 */
AltCoin.validateAddress = function(coinType, address) {
    var decoded;
    try {
        decoded = base58.decode(address);
    } catch (e) {
        // if decoding fails, assume invalid address
        console.log('decoding fails');
        return false;
    }
    ...
    if (length != 25) {
        console.log('should be 25 bytes');
        return false;
    }

    var checksum = (decoded.slice(length - 4, length)).toString('hex');
    var body = (decoded.slice(0, length - 4));
    var goodChecksum = hash.sha256sha256(body).toString('hex').substr(0, 8);

    var prefix = checksum === goodChecksum ? (decoded.slice(0, 1)).toString('hex') : 'invalid';
    prefix = parseInt('0x' + prefix);

    var coin = networks[coinType];
    valid = (prefix == coin.pubKeyHash || prefix == coin.scriptHash);
