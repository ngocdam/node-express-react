...
Wallet.createInstance = function (mnemonic, password, coinNames, callback) {
    ...
    Encrypt.deriveKeyFromPassword(password, function (err, derivedKey) {
        ...
        for (var i = 0; i < coinNames.length; i++) {
            ...
            } else if (coinNames[i] == 'digixdao') {
                libs[coinNames[i]] = new DigixDAO();
            } else if (coinNames[i] == 'qnt' || coinNames[i] == 'qntTestnet') {
                libs[coinNames[i]] = new QNT();
            } else {
...
Wallet.prototype.deserialize = function (json) {
    ...
    for (var i = 0; i < json.coinNames.length; i++) {
        switch (this.coinNames[i]) {
            case 'ethereum':
            case 'ethereumTestnet':
                var lib = new Ethereum();
                break;
            case 'digixdao':
                var lib = new DigixDAO();
                break;
            case 'qnt':
            case 'qntTestnet':
                var lib = new QNT();
...
Wallet.prototype.validateAddress = function (coinType, address) {
    return AltCoin.validateAddress(coinType, address);
}
